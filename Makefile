...
# these variables are the single source of truth. It also keeps the stamp off
# the untracked-file noise (e.g. cmd/gc/dashboard) that otherwise flips
# vcs.modified=true and makes a clean-tree build report `-dirty`.
...
build:
	...
	@set -e; \
		...
		go build -buildvcs=false -ldflags "$(LDFLAGS)" -o "$$tmp" ./cmd/gc; \
