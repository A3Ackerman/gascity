...
| `ControllerStopped` | `controller.stopped` | Per-city controller shutdown. Carries a typed reason (`supervisor_shutdown`, `user_stop`, `restart`, `crash`, `drain`, `config_reload`, or `unknown`), the parent `supervisor.shutdown_requested` sequence when applicable, and optional crash detail. |
