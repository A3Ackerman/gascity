...
func shouldEmitLoadCityConfigWarning(warning string) bool {
	...
	// choice, not a condition of this invocation, so emitting it inline puts
	// unconditional stderr lines above the output of EVERY gc command in every
	// agent's context (the bundled gastown pack alone trips it three times —
	// see TestGastownSmoke's expectedWarningConditions). That is worse than
	// untidy: an agent read the
