...
var controllerTestExcludedHangDeadlineLines = map[int]string{
	481:  "input the test feeds a fake server to define the scenario, not a hang detector",
	937:  "negative-assertion window (asserts no watcher poke arrives)",
	987:  "negative-assertion window (asserts no watcher poke arrives, loop body)",
	1516: "bounded best-effort probe with no assertion on either branch",
}
