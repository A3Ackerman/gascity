...
| Medium owner | `internal/packman` package `packman` | TestGcHomeSplitWedgeIsRefused: subprocess | gastownhall/gascity#5506 | the GC_HOME-split cache-root wedge refusal proof is a checked Medium subprocess owner; the git subprocesses are confined to TestGcHomeSplitWedgeIsRefused, which proves end-to-end that a foreign-rooted install is refused against a real clone layout: the cache-root binding records where real clones landed, so only a real repository can prove the refusal fires before the cache or lockfile is touched | P0.4b | 2026-10-01 |
